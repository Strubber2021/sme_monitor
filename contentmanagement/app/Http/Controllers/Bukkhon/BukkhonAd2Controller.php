...
class BukkhonAd2Controller extends Controller
{
    ...
    {
        try {
            $getAllImageOriginal = BukkhonAd2Service::getAllImage();
            ...
            $data = [
                ...
                'page_name' => 'Bukkhon/ad2',
                'side_name' => 'bukkhon',
                'page_name_th' => 'หน้าหลัก',
			    'page_name_th_url' => '/bukkhon',
                'page_name_v1' => 'ad2',
                'page_name_v1_url' => '/bukkhon/ads/2',
                'page_name_v2' => null,
                'page_name_v2_url' => null,
                'imgshowup' => $getAllImageOriginal
            ];
            return view('pages.bukkhon.bukkhon-ads-2')->with($data);
        } catch (\Throwable $th) {
            throw $th;
        }
    }
    ...
    public function storeImg(Request $request)
    {
        try {
            ...
            $request->image->move(public_path('assets/images/bukkhon/ad2'), $imageName);
            $result =  BukkhonAd2Service::storeImg($imageName);
    ...
    {
        try {
        $path =  public_path("/assets/images/bukkhon/ad2/").$imgName;
        $isExists =  File::exists($path);
        
        if ($isExists) {
            unlink($path);
        }
        $result =  BukkhonAd2Service::destroyImg($imgId);
            return JsonResult::success($result);
        } catch (\Throwable $e) {
            return JsonResult::errors($e->getMessage(), AppConstants::ERROR_CONTACT_ADMIN);
        }
