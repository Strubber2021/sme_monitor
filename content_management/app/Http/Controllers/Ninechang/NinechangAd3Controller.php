...
class NinechangAd3Controller extends Controller
{
    ...
    {
        try {
            $getAllImageOriginal = NinechangAd3Service::getAllImage();
            ...
            $data = [
                ...
                'page_name' => 'Ninechang/ad3',
                'side_name' => 'ninechang',
                'page_name_th' => 'หน้าหลัก',
			    'page_name_th_url' => '/ninechang',
                'page_name_v1' => 'ad3',
                'page_name_v1_url' => '/ninechang/ads/3',
                'page_name_v2' => null,
                'page_name_v2_url' => null,
                'imgshowup' => $getAllImageOriginal
            ];
            return view('pages.ninechang.ninechang-ads-3')->with($data);
        } catch (\Throwable $th) {
            throw $th;
        }
    }
    ...
    public function storeImg(Request $request)
    {
        try {
            ...
            $request->image->move(public_path('assets/images/ninechang/ad3'), $imageName);
            $result =  NinechangAd3Service::storeImg($imageName);
    ...
    {
        try {
            $path =  public_path("/assets/images/ninechang/ad3/").$imgName;
            $isExists =  File::exists($path);
            
            if ($isExists) {
                unlink($path);
            }
            $result =  NinechangAd3Service::destroyImg($imgId);
        return JsonResult::success($result);
        } catch (\Throwable $e) {
            return JsonResult::errors($e->getMessage(), AppConstants::ERROR_CONTACT_ADMIN);
        }
