...
class NinechangAd1Controller extends Controller
{
    ...
    {
        try {
            $getAllImageOriginal = NinechangAd1Service::getAllImage();
            ...
            $data = [
                ...
                'page_name' => 'Ninechang/ad1',
                'side_name' => 'ninechang',
                'page_name_th' => 'หน้าหลัก',
			    'page_name_th_url' => '/ninechang',
                'page_name_v1' => 'ad1',
                'page_name_v1_url' => '/ninechang/ads/1',
                'page_name_v2' => null,
                'page_name_v2_url' => null,
                'imgshowup' => $getAllImageOriginal
            ];
            return view('pages.ninechang.ninechang-ads-1')->with($data);
        } catch (\Throwable $th) {
            throw $th;
        }
    }
    ...
    public function storeImg(Request $request)
    {
        try {
            ...
            $request->image->move(public_path('assets/images/ninechang/ad1'), $imageName);
            $result =  NinechangAd1Service::storeImg($imageName);
    ...
    {
        try {
        $path =  public_path("/assets/images/ninechang/ad1/").$imgName;
        $isExists =  File::exists($path);
        
        if ($isExists) {
            unlink($path);
        }
        $result =  NinechangAd1Service::destroyImg($imgId);
            return JsonResult::success($result);
        } catch (\Throwable $e) {
            return JsonResult::errors($e->getMessage(), AppConstants::ERROR_CONTACT_ADMIN);
        }
